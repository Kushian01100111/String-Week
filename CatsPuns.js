...
function catPuns(str){
    let canineFilter = ['DOG','Dog','dog','BARK','Bark','bark','BONE','Bone','bone']
    let first = str.split(",").map(n => n = n.trim()).filter(n =>{
        let boolean ;
        canineFilter.every(x =>{
            if(n.includes(x)){
                boolean = true
                return false 
            }else return true
        })
        return boolean ? null : n 
    }).filter(n =>{
        return n.length % 5 === 0 ? null : n 
    }).filter(n =>{
        let sum = [n.charCodeAt(), n.charCodeAt(n.length-1)].reduce((acc,c) => acc + c , 0)
        return sum % 2 === 1 ? null : n
    }).filter(n =>{
        return n.charAt(((n.length-1)/2)+1) === 'e' ? null : n
    }).filter(n =>{
        let split = n.split(""),
            even = 0;
            split.forEach(n =>{
                n === n.toLowerCase() && n !== ' ' && n !== ':' ? even++ : null
            })
        return even % 2 === 0 ? n : null
    }).filter(n =>{
        let split = n.split(""),
            atLeast = 0;
            split.forEach(n =>{
                n === n.toUpperCase() && n !== ' ' && n !== ':' ? atLeast++ : null
            })
        return atLeast >= 2 ? n : null
    }).filter(n=>{
        return n.includes('S') ? null : n
    })

    return first
}

console.log(catPuns("The Pawshank Redemption,Caturday Night Live,Only Meworders in the Building,I Love Mewcy, Mewsummer Meowders,The Golden Purrs, Purrlandia ,Meowpardy, Meowstery Science Theater: Purrthousand, Amewican Idol,Dog City,Doctor Mew , The Meowing Fed,Mew Peter,The Vicar of Dogley, Kittens,Meownton Abbey,Pets and the Kitty,Dogis and Bonehead,Pawlty Meowers ,The Meowpet Show,Barkos,The Catbert Purrport,The Pawffice,The Dogford Files, Battlestar Catlactica,Catlumbo,SpongeDog Squarepants,NYPD Mew ,Fluffy the Meowpire Purrer,The Inbemewners,Meowder She Wrote,Paw & Order,30 Dog, Pawvatar: The Last Meowbender,The Pawnight Show,Arrested Dogvelopment,Furiends,Mewie,Curb Your Dogthusiasm,Teenage Mewtant Ninja Turtles,Phineas and Purrb,Paw Trek, Paw Trek: The Next Mewination, Twin Mewks, *C*A*T*S*,DogTales, Game of Bones, House of the Meowgon,The Purrlight Zone,Breaking Bone,The Meowre,The Dogpranos,The Rings of Meower, The KIT Crowd,Strangepaw Things ,Catman: The Animeowted Series,Meowter Call Saul,Mewgerton ,Obark,Mewphoria,La Casa de Pawpel,Rick & Meowty,Amewican Purror Story, Mewcifer,PawndaVision,Dogxter,The Meowndalorian, Dog Lasso,Bark,Meowdern Pawmily , Meowtlander,Bone Mirror,Barks and Recreation,How to Get Away with Meowder,Boneland ,Meowther Ted,Mewtopia,Mewey,The Mewkie Meowse Doghouse,Mewster Rogers' Neighborhood"))